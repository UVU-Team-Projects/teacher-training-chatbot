...
from .embedding import EmbeddingGenerator  # Use relative import with .
import json
import logging
...
class TeacherTrainingChatbot:
    ...
    def retrieve_context(self, query: str, k: int = 5) -> str:
        ...
        try:
            ...
            # Try using chroma's normal API first
            try:
                results = db.similarity_search_with_score(query, k=k)
                context = "\n\n---\n\n".join([doc.page_content for doc, _score in results])
            except (AttributeError, TypeError):
                # Fallback for when similarity_search_with_score isn't available or returns incorrect type
                context = ""
            return context
        except Exception as e:
            logger.error(f"Error retrieving context: {e}")
            return ""  # Return empty string on error rather than crashing
